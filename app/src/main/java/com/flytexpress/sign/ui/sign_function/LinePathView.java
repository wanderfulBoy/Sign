...
public class LinePathView extends View {
    ...
    public void save(String path, boolean clearBlank, int blank) throws IOException {
        //添加水印签名
        Bitmap bitmap= createWatermark(cachebBitmap,"本签名仅用于做“飞特运单签收”如有他用一律非法无效");
//        Bitmap bitmap=cachebBitmap;20180117屏蔽
    ...
    private Bitmap createWatermark(Bitmap target, String mark) {
        ...
        // 水印的颜色
        p.setColor(getResources().getColor(android.R.color.darker_gray));
        ...
        int width=(int)p.measureText(mark,0,25);
        Log.e("width:","width:"+w+","+width+"zhih"+(w+width*16/2));
        Paint.FontMetricsInt fmi = p.getFontMetricsInt();

        int textCenterVerticalBaselineY = h + (fmi.descent - fmi.ascent) / 2- fmi.descent ;
        int textCenterVerticalBaseLineX= w +(fmi.descent-fmi.ascent)/2 - fmi.ascent;
        // 在左边的中间位置开始添加水印
        canvas.drawText(mark, (w-width)/2, h-16, p);
